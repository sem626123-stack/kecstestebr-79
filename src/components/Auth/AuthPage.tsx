...
import MobileOrientationPermission from './MobileOrientationPermission';
...
const AuthPage = ({ onBack }: AuthPageProps) => {
  ...
  const [deviceOrientation, setDeviceOrientation] = useState({ tiltX: 0, tiltY: 0 });
  ...
  const initializeInteraction = () => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const x = (e.clientX / rect - 0.5) * 2; // Convert to -1 to 1 range
      const y = (e.clientY / height - 0.5) * 2; // Convert to -1 to 1 range
    ...
    const handleOrientation = (e: DeviceOrientationEvent) => {
      if (e.gamma !== null && e.beta !== null) {
        ...
        const tiltX = Math.max(-1, Math.min(1, e.gamma / 45)); // gamma is left-right tilt
        const tiltY = Math.max(-1, Math.min(1, (e.beta - 90) / 45)); // beta is front-back tilt, normalize around 90°
        setDeviceOrientation({ tiltX, tiltY });
      }
    };
        ...
        <div className="flex min-h-screen">
          ...
          <div className="hidden lg:flex lg:w-1/2 items-center justify-center relative">
            <div className="text-center space-y-8 px-8">
              {/* Character with Interactive Animation */}
              <div 
                ref={characterRef}
              ...
              >
                <div className="relative">
                  <div 
                    className="w-80 h-auto mx-auto transition-transform duration-300 ease-out"
                    style={{
                      transform: `
                        translateX(${(mousePosition.x * 15) + (deviceOrientation.tiltX * 15)}px) 
                        translateY(${(mousePosition.y * 10) + (deviceOrientation.tiltY * 10)}px)
                        rotateY(${(mousePosition.x * 8) + (deviceOrientation.tiltX * 8)}deg)
                        rotateX(${(mousePosition.y * -5) + (deviceOrientation.tiltY * -5)}deg)
                      `
                    }}
                  >
                    {/* Character Image with CSS Animation for body movement */}
                    <img 
                      src={loginCharacter} 
                      alt="Bem-vindo" 
                      className="w-full h-auto animate-float-gentle filter drop-shadow-2xl"
                      style={{
                        transform: `
                          rotate(${(mousePosition.x * 3) + (deviceOrientation.tiltX * 3)}deg)
                        `
                      }}
                    />
                    
                    {/* Animated Eyes following movement */}
                    <div 
                      className="absolute top-[22%] left-[42%] w-2 h-2 bg-primary rounded-full transition-transform duration-200"
                      style={{
                        transform: `translate(${(mousePosition.x * 2) + (deviceOrientation.tiltX * 2)}px, ${(mousePosition.y * 1) + (deviceOrientation.tiltY * 1)}px)`
                      }}
                    />
                    <div 
                      className="absolute top-[22%] right-[42%] w-2 h-2 bg-primary rounded-full transition-transform duration-200"
                      style={{
                        transform: `translate(${(mousePosition.x * 2) + (deviceOrientation.tiltX * 2)}px, ${(mousePosition.y * 1) + (deviceOrientation.tiltY * 1)}px)`
                      }}
                    />
                  ...
                  {/* Enhanced Glow Effect that reacts to movement */}
                  <div 
                    className="absolute inset-0 bg-gradient-primary opacity-20 blur-3xl rounded-full scale-75 animate-pulse-soft transition-transform duration-500"
                    style={{
                      transform: `
                        translateX(${(mousePosition.x * 10) + (deviceOrientation.tiltX * 10)}px) 
                        translateY(${(mousePosition.y * 8) + (deviceOrientation.tiltY * 8)}px)
                        scale(${0.75 + Math.abs(mousePosition.x * 0.1) + Math.abs(deviceOrientation.tiltX * 0.1)})
                      `
                    }}
                  />
                </div>
              ...
              {/* Welcome Text with movement hint */}
              <div className={`space-y-4 transition-all duration-1000 delay-300 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                ...
                <p className="text-sm text-primary/70 animate-pulse">
                  💫 Mova o mouse ou incline o celular para interagir!
                </p>
              </div>
